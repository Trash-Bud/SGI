...
import { MyPatch } from '../tp2/MyPatch.js';
            ...
            if (primitiveType == 'rectangle') {
                ...
                for(let u = 0; u < degree_u + 1; u++){
                    ...
                    for(let v = 0; v < degree_v + 1; v++ ){
                        v_subArray.push([...control_points[u * (degree_v + 1) + v], 1])
                    }
                    real_control_points.push(v_subArray)
                }

                
                var patch = new MyPatch(this.scene, degree_u,degree_v, parts_u,parts_v,real_control_points);
    ...
    parseComponents(componentsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (animationsIndex != -1){
                ...
                for (let transfCount = 0; transfCount < animation.keyframes[0].transformation.length; transfCount++){
                    var transformation = animation.keyframes[0].transformation[transfCount]
                    componentObj["animationTransformations"].push(transformation)
                }
            }
            // Transformations
